 ...
 * (R2 is S3-compatible; works against R2 or self-hosted MinIO).
 */
...
export class S3R2Bucket {
  ...
  async get(key: string): Promise<{
    ...
    try {
      const res = await getClient().send(new GetObjectCommand({ Bucket: this.bucket, Key: key }));
...
const buckets = new Map<string, S3R2Bucket>();

export function getR2Bucket(bucketName: string): S3R2Bucket {
  let bucket = buckets.get(bucketName);
  if (!bucket) {
    bucket = new S3R2Bucket(bucketName);
