...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const response = NextResponse.json({ error: `Failed to create template: ${error instanceof Error ? error.message : 'Unknown error'}` }, { status: 500 });
    return withSecurityHeaders(response, traceId);
  }
}
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const response = NextResponse.json({ error: `Failed to fetch templates: ${error instanceof Error ? error.message : 'Unknown error'}` }, { status: 500 });
    return withSecurityHeaders(response, traceId);
  }
}
...
export async function DELETE(request: NextRequest) {
  ...
  try {
    ...
    const response = NextResponse.json({ error: `Failed to delete template: ${error instanceof Error ? error.message : 'Unknown error'}` }, { status: 500 });
    return withSecurityHeaders(response, traceId);
  }
}
