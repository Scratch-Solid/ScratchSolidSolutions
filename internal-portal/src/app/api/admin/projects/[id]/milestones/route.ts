...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const resp = await proxy(`${apiBase}/api/projects/${id}/milestones`, { method: 'POST', headers: upstreamHeaders(), body: JSON.stringify(body) });
...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { id } = await params;
  const body = await request.json();
  const resp = await proxy(`${apiBase}/api/projects/${id}/milestones`, { method: 'PUT', headers: upstreamHeaders(), body: JSON.stringify(body) });
  return withSecurityHeaders(resp, traceId);
}
