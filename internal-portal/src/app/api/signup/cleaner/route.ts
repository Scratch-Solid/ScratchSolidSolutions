...
import { encryptField, hashData } from '@/lib/encryption';
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const encryptedIdNumber = await encryptField(id_number);
    const encryptedPhone = await encryptField(phone);
    const encryptedEmergencyContact = await encryptField(sanitizedEmergencyContact);
    const encryptedBankDetails = await encryptField(sanitizedBankDetails);
    const encryptedWhatsapp = await encryptField(sanitizedWhatsapp);

    // Check if ID number already exists (compare hashes since encrypted values are non-deterministic)
    const idHash = await hashData(id_number);
    const existingIdHash = await db.prepare(
      'SELECT id FROM new_joiners WHERE id_number_hash = ?'
    ).bind(idHash).first();
    ...
    await db.prepare(
      `INSERT INTO new_joiners (
        name, id_number, id_number_hash, email, phone, whatsapp, address, emergency_contact,
