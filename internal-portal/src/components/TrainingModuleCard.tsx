...
export default function TrainingModuleCard({
  ...
  return (
    <div className="w-full max-w-md mx-auto bg-white rounded-xl shadow-md overflow-hidden md:max-w-2xl border border-slate-100 my-4">
      <div className="p-6">
        {/* Header Block */}
        ...
        <h3 className="block text-lg leading-tight font-bold text-slate-900 mb-2">
          {moduleTitle}
        </h3>
        ...
        {status === 'locked' && timeRemaining && (
          <div className="bg-slate-50 border border-slate-200 rounded-lg p-4 mb-4 flex items-start gap-3">
            ...
            <div>
              <h4 className="text-xs font-bold text-slate-700 uppercase tracking-wider">Module Locked</h4>
              <p className="text-xs text-slate-500 mt-0.5">
                To prevent training fatigue, this module unlocks in <span className="font-mono font-bold text-slate-800">{timeRemaining}</span>.
              </p>
            </div>
          </div>
        )}
        ...
        {status === 'active' && (
          <button
            onClick={onStart}
            className="w-full bg-slate-900 hover:bg-slate-800 text-white font-medium text-sm py-3 px-4 rounded-lg transition duration-150 ease-in-out flex items-center justify-center gap-2 shadow-sm focus:outline-none focus:ring-2 focus:ring-slate-500 focus:ring-offset-2"
          >
        ...
        {status === 'locked' && !timeRemaining && (
          <button
            disabled
            className="w-full bg-slate-100 text-slate-500 font-medium text-sm py-3 px-4 rounded-lg flex items-center justify-center gap-2 cursor-not-allowed"
          >
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
