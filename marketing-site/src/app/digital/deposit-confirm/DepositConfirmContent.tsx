...
export default function DepositConfirmContent() {
  ...
  const [message, setMessage] = useState("Confirming your deposit...");
  ...
  useEffect(() => {
    const verify = async () => {
      ...
      try {
        const response = await fetch(`/api/intake/${intakeId}/deposit/verify?reference=${encodeURIComponent(reference)}`);
        const data = await response.json() as { status?: string; message?: string };

        if (response.ok && data.status === "success") {
          setStatus("success");
          setMessage(data.message || "Deposit confirmed. Our team will be in touch to kick off your project.");
        } else {
          setStatus("failed");
          setMessage(data.message || "We couldn't confirm your deposit. Please contact us if you were charged.");
        }
      } catch {
        setStatus("failed");
        setMessage("Network error while confirming your deposit. Please contact us if you were charged.");
      }
    };
        ...
        <div className="bg-white/90 backdrop-blur-md rounded-2xl shadow-2xl border-2 border-white/20 p-8">
          {status === "verifying" && (
            <>
              ...
              <h2 className="text-2xl font-semibold text-gray-800 mb-2">Confirming Your Deposit</h2>
          ...
          {status === "success" && (
            <>
              ...
              <h2 className="text-2xl font-semibold text-green-700 mb-2">Deposit Received!</h2>
              <p className="text-gray-600">{message}</p>
              <p className="text-sm text-gray-500 mt-4">You'll also get a confirmation email with your payment details.</p>
