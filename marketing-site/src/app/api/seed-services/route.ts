...
export async function POST(request: NextRequest) {
  try {
    ...
    await db.prepare(
      ...
      ('Standard Clean', 'Regular cleaning service', 'Our standard cleaning service includes dusting, vacuuming, mopping, bathroom cleaning, and kitchen cleaning. Perfect for regular maintenance.', 350.00, 1.0, 1, 'S', 1, datetime('now'), datetime('now')),
      ('Deep Clean', 'Deep cleaning service', 'Deep cleaning service includes everything in standard clean plus detailed cleaning of all surfaces, inside cabinets, baseboards, and hard-to-reach areas.', 550.00, 1.2, 1, 'D', 2, datetime('now'), datetime('now')),
      ('Move In/Out', 'Move in or move out cleaning', 'Comprehensive cleaning for moving in or out. Includes all surfaces, appliances, cabinets, and ensures the property is spotless for the next occupant.', 750.00, 1.5, 1, 'M', 3, datetime('now'), datetime('now')),
      ('Post-Construction', 'Post-construction cleaning', 'Specialized cleaning after construction or renovation. Removes dust, debris, and prepares the space for occupancy.', 1200.00, 2.0, 1, 'C', 4, datetime('now'), datetime('now')),
      ('Commercial', 'Commercial cleaning', 'Professional cleaning for offices, retail spaces, and commercial properties. Customized to your business needs.', 450.00, 1.3, 1, 'B', 5, datetime('now'), datetime('now')),
      ('Maintenance Clean', 'Weekly or bi-weekly maintenance cleaning', 'Regular maintenance cleaning for homes and offices. Ideal for ongoing cleaning schedules.', 300.00, 1.0, 1, 'T', 6, datetime('now'), datetime('now')),
      ('Short-Term Stay', 'LekkeSlaap and short-term rental cleaning', 'Five-star guest turnovers for local rentals. Ensures your property stays "Lekke" for every guest.', 400.00, 1.1, 1, 'L', 7, datetime('now'), datetime('now'))`
    ).run();
