...
export function Table<T>({ data, columns, keyExtractor }: TableProps<T>) {
  return (
    <div className="overflow-x-auto rounded-xl border border-[#E9E0D3] shadow-sm">
      <table className="w-full text-sm">
        <thead className="bg-[#F7F2EA]/80 backdrop-blur-sm">
          <tr>
            {columns.map(col => (
        ...
        <tbody className="divide-y divide-gray-100">
          {data.map(row => (
            <tr key={keyExtractor(row)} className="hover:bg-[#F7F2EA]/40 transition-colors">
              {columns.map(col => (
