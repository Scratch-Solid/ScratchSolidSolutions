  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-lg font-bold text-gray-800">My Quotes</h3>
        <p className="text-xs text-gray-500">Logged in as {userEmail}</p>
      </div>

      {quotes.length === 0 ? (
        <div className="text-center py-8">
          <p className="text-gray-600">No quotes yet. Request your first quote today!</p>
        </div>
      ) : (
          ...
          {quotes.map((quote) => (
            <div key={quote.id} className="bg-gray-50 border border-gray-200 rounded-lg p-4">
              <div className="flex items-start justify-between mb-3">
                <div>
                  <p className="font-semibold text-gray-900 text-sm">Ref: {quote.ref_number}</p>
              ...
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-gray-600 text-sm">
                    ...
                    <span className="font-bold text-gray-900">R{quote.final_price.toFixed(2)}</span>
                  </p>
                  {quote.promo_code && (
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className="text-blue-600 hover:text-blue-700 px-3 py-1 border border-blue-500/30 rounded transition-all text-xs"
                  >
                    Download PDF
                  </button>
                  {quote.status === 'pending' && !isQuoteExpired(quote.valid_until) && (
                    <button
                      ...
                      className="text-green-600 hover:text-green-700 px-3 py-1 border border-green-500/30 rounded transition-all text-xs"
                    >
                      Accept Quote
                    </button>
                  )}
